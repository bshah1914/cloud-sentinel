...
export default function FindingsTrend({ account, provider }) {
      ...
      .then((d) => {
        const scans = (d.scans || d.history || d || [])
          ...
          .map((s) => ({
            ...
            findings: s.total_findings ?? s.findings_count ?? 0,
          }));
        setData(scans);
      })
      .catch(() => setData([]))
  ...
  if (!data.length) return <div className="flex items-center justify-center h-full text-text-muted text-xs">No trend data</div>;
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={data} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
        ...
        <Area type="monotone" dataKey="findings" stroke="#f97316" fill="#f97316" fillOpacity={0.2} />
      </AreaChart>
    </ResponsiveContainer>
  );
}
